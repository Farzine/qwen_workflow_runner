...
import inspect
import math
# Modified for the supplied workflow: first-reference canvas, independent reference sizes,
# FP32 latent accumulation, and CFG below one. Original source recorded in workflow/source_manifest.json.
...
class WorkflowQwenImage21Pipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    r"""
    ...
    def _get_qwen_prompt_embeds(
        ...
        if not is_t2i:
            ...
            # Already sized to the 32px latent/vision grid. A second resize in
            # Qwen3-VL would desynchronize its image slots from the VAE latents.
    ...
    def __call__(
        ...
        kv_cache_reserve_gib: float = 1.0,
    ):
        r"""
        ...
        Args:
            ...
            output_resolution (`int`, *optional*, defaults to 1024):
                Fallback canvas side when no reference or explicit dimensions are supplied.
                Condition images must already be resized by qwen_runner.images; they are not rescaled here.
            ...
            kv_cache_device (`str`, *optional*, defaults to `"auto"`):
                Lossless prefix storage: auto, gpu (execution device), or cpu.
            kv_cache_reserve_gib (`float`, *optional*, defaults to 1):
                Free CUDA memory reserve used by the auto placement policy.
        ...
        if image is not None:
            ...
            # Workflow patch: image1 is the canvas; references were resized once by images.py.
            calculated_width, calculated_height = image[0].size
        ...
        if image is not None:
            ...
            for img in image:
                ...
                # Workflow patch: preserve each independently prepared reference size.
        ...
        cond_cache = PrefixCache(num_blocks, kv_cache_device, int(kv_cache_reserve_gib * 2**30)) if cache_enabled else None
        neg_cache = PrefixCache(num_blocks, kv_cache_device, int(kv_cache_reserve_gib * 2**30)) if cache_enabled and do_true_cfg else None
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # Keep Euler accumulation FP32 and cast only model input.
