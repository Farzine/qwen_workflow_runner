...
class ModelStore:
    ...
    def fetch(self, ref, purpose="pipeline", quantization="Q4_K_M"):
        if ref.local:
            p = Path(ref.local)
        ...
        for name in selected:
            p = Path(download(repo_id=ref.repo_id, filename=name, revision=commit, cache_dir=str(self.root / 'hub'), local_files_only=False))
