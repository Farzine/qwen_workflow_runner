...
|---|------|-------|-------------|--------|
| M1 | Backend API & Runner Integration | Fast backend server (`ui/server.py`, `ui/api.py`), endpoints for inputs scanning, thumbnail caching, model upload/download/caching, parameter validation via `Config.validate()`, background runner execution with SSE log streaming, and demo backend fallback | none | PLANNED |
